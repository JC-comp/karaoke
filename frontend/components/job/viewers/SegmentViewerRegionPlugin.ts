...
class RegionsPlugin extends BasePlugin<RegionsPluginEvents, RegionsPluginOptions> {
  ...
  private reRenderTimeout: NodeJS.Timeout | null = null
  private onZoomTimeout: NodeJS.Timeout | null = null
      ...
      this.wavesurfer.on('zoom', () => {
        ...
        this.onZoomTimeout = setTimeout(() => {
          ...
          displayedRegions.forEach((region) => {
            region.content?.setAttribute('data-computed', 'false')
          })
          displayedRegions.forEach((region) => {
  ...
  private avoidOverlapping(region: Region) {
    ...
    setTimeout(() => {
      ...
      if (!region.element?.parentElement) return

      // Recompute height of the player
      if (this.reRenderTimeout) {
        clearTimeout(this.reRenderTimeout)
      }
      this.reRenderTimeout = setTimeout(() => {
        const newHeight = Math.max(128, ...this.regions.map((reg) => {
          if (!reg.content) return 0;
          if (!reg.element?.parentElement) return 0;
          var marginTop = parseFloat(reg.content.style.top || '0')
          marginTop += reg.content.getBoundingClientRect().height
          return marginTop
        }));
        this.wavesurfer?.setOptions({ height: newHeight })
      }, 1000)
      ...
      var ready = target_direction
        .filter((reg) => {
          if (reg.content === undefined)
            return false
          return reg.content.dataset.computed !== 'true'
        }).length === 0
      ...
      while (overlap > 0) {
        overlap = Math.max(...preceding
          .map((reg) => {
            if (reg === region || !reg.content) return 0

            const otherBox = reg.content.getBoundingClientRect()
            if ((box.bottom + accumulatedTop) <= otherBox.top || (box.top + accumulatedTop) >= otherBox.bottom) {
              return 0
            }
            if (box.left < otherBox.left + otherBox.width && otherBox.left < box.left + box.width) {
              return otherBox.height + otherBox.top - (box.top + accumulatedTop)
            }
            return 0
          })
        )
        if (overlap > 0)
          accumulatedTop += overlap;
      }

      if (accumulatedTop > 0) {
        const oldMargin = parseFloat(div.style.top || '0') + 0.1
        const newMargin = oldMargin + accumulatedTop
        div.style.top = `${newMargin}px`
      }

      div.dataset.computed = 'true'
      following.filter((reg) => reg.content?.dataset.computed === 'true')
        .forEach((reg) => {
          ...
          if (box.left < otherBox.left + otherBox.width && otherBox.left < box.left + box.width) {
            reg.content.style.top = "0"
            reg.content.setAttribute('data-computed', 'false')
            this.avoidOverlapping(reg)
          }
        });
    }, 10)
  }
  ...
  private virtualAppend(region: Region, container: HTMLElement, element: HTMLElement) {
    const renderIfVisible = () => {
      ...
      if (isVisible && !element.parentElement) {
        ...
        // Remove the element from the DOM and set the computation of overlapping to false
        const content = region.content;
        if (content) {
          content.setAttribute('data-computed', 'false')
    ...
    setTimeout(() => {
      ...
      const unsubscribeRedraw = this.wavesurfer.on('redraw', renderIfVisible);
      this.subscriptions.push(unsubscribeRedraw)
    }, 0)
  }
